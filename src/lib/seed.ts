...
import { identityKey, variantKey } from "./catalogue";

/*
 * Starter dataset: the product catalogue (products, categories, attribute
 * definitions and options, stocked variants) plus warehouse structure, and
 * ~3 months of demo trading history — suppliers, customers, stock lots,
 * invoices, payments and expenses. Dates are computed relative to "today"
 * so dashboards and reports always show a recent, coherent period.
 */
...
export const seedProducts: Product[] = [
  { id: "prod-steel", businessId: BUSINESS_ID, name: "Steel", unit: "kg", active: true, usesCategories: false },
  { id: "prod-wire", businessId: BUSINESS_ID, name: "Wire", unit: "kg", active: true, usesCategories: true },
  { id: "prod-cement", businessId: BUSINESS_ID, name: "Cement", unit: "bag", active: true, usesCategories: true },
];
...
export const seedAttributeDefs: AttributeDef[] = [
  { id: "def-rebar-size", businessId: BUSINESS_ID, productId: "prod-steel", name: "Size", key: "size", type: "select", required: true, sortOrder: 1, active: true },
  { id: "def-rebar-grade", businessId: BUSINESS_ID, productId: "prod-steel", name: "Grade", key: "grade", type: "select", required: false, sortOrder: 2, active: true },
  { id: "def-wire-gauge", businessId: BUSINESS_ID, productId: "prod-wire", categoryId: "cat-wire-ba", name: "Gauge", key: "gauge", type: "select", required: true, sortOrder: 1, active: true },
  { id: "def-gi-gauge", businessId: BUSINESS_ID, productId: "prod-wire", categoryId: "cat-wire-gi", name: "Gauge", key: "gauge", type: "select", required: true, sortOrder: 1, active: true },
  { id: "def-grey-brand", businessId: BUSINESS_ID, productId: "prod-cement", categoryId: "cat-cement-grey", name: "Brand", key: "brand", type: "select", required: true, sortOrder: 1, active: true },
  { id: "def-grey-grade", businessId: BUSINESS_ID, productId: "prod-cement", categoryId: "cat-cement-grey", name: "Grade", key: "grade", type: "select", required: false, sortOrder: 2, active: true },
  { id: "def-white-brand", businessId: BUSINESS_ID, productId: "prod-cement", categoryId: "cat-cement-white", name: "Brand", key: "brand", type: "select", required: true, sortOrder: 1, active: true },
  { id: "def-white-grade", businessId: BUSINESS_ID, productId: "prod-cement", categoryId: "cat-cement-white", name: "Grade", key: "grade", type: "select", required: false, sortOrder: 2, active: true },
];
...
const CAT_PRODUCT: Record<string, string> = {
  "cat-rebar": "prod-steel",
  "cat-wire-ba": "prod-wire",
  "cat-wire-gi": "prod-wire",
  "cat-cement-grey": "prod-cement",
  "cat-cement-white": "prod-cement",
};
...
const mkVariant = (
  ...
  const productId = CAT_PRODUCT[categoryId];
  const usesCat = seedProducts.find((p) => p.id === productId)?.usesCategories !== false;
  const scopeId = usesCat ? categoryId : productId;
  const defs = seedAttributeDefs.filter((d) =>
    usesCat ? d.categoryId === categoryId : d.productId === productId && !d.categoryId
  );
  return {
    id,
    businessId: BUSINESS_ID,
    productId,
    categoryId: usesCat ? categoryId : undefined,
    key: variantKey(categoryId, attributes), // keep original category-scoped key so old lookups still hit
    identityKey: identityKey(scopeId, attributes, defs, seedAttributeOptions),
    attributes,
    shortName,
    active: true,
...
export const seedPurchases: Purchase[] = [
  ...
  mkPurchase("pur-16", 2, "sup-mughal", "var-rebar-5s40", "5 Sutar · 40 Grade", "Steel", { size: "5 Sutar", grade: "40 Grade" },
    1000, "kg", 265, { transport: 9000, loading: 3000, labour: 2500, other: 0, sellRate: 295, paid: 265000 }, { locationId: "loc-steel", heatNumber: "M-53102" }),
  mkPurchase("pur-17", 2, "sup-ittefaq", "var-ba-18", "18 Gauge", "Wire", { gauge: "18 Gauge" },
    600, "kg", 288, { transport: 5000, loading: 1500, labour: 1000, other: 0, sellRate: 318, paid: 172800 }, { locationId: "loc-steel", lotNumber: "LOT-B18-03" }),
  mkPurchase("pur-18", 6, "sup-dgkhan", "var-grey-lucky53", "Lucky Cement · 53 OPC", "Cement", { brand: "Lucky Cement", grade: "53 OPC" },
    400, "bag", 1400, { transport: 16000, loading: 3500, labour: 2500, other: 0, sellRate: 1540, paid: 560000 }, { locationId: "loc-cement", batchNumber: "LC-78900" }),
];
