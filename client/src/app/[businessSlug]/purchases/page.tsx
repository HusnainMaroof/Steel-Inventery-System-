...
export default function PurchasesPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setFormError("");
    const variant = await ensureVariant(product.id, usesCats ? category?.id : undefined, form.attrs);
    await addPurchase({
      date: form.date,
  ...
  return (
    <Page>
      ...
      {tab === "all" && (
        purchases.length === 0 ? (
          <EmptyState emoji="🚚" title="No purchases yet" hint={activeSuppliers.length === 0 ? "Add a mill / supplier first, then your first purchase will feel right at home." : "Record your first purchase — date, supplier and rate is all it takes."} action={<button className="btn-primary" onClick={openAdd}>+ Add Purchase</button>} />
        ...
        {(() => {
          ...
          return (
            <form noValidate onSubmit={async (e) => {
              ...
              if (amt <= 0) {
                setPayError("Enter the amount you're paying to the mill.");
                return;
              }
              if (amt > rem + 0.001) {
                setPayError(`You can't pay more than the remaining due of ${fmtMoney(rem)} — ${fmtMoney(total)} is payable and ${fmtMoney(paid)} is already paid.`);
                return;
              }
              setPayError("");
